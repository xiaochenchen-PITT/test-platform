 ...
 */
@Configuration
@MapperScan(basePackages = "com.cxc.test.platform", sqlSessionFactoryRef = "xytestSqlSessionFactory")
public class XytestDateSourceConfig {

    @Primary // 表示这个数据源是默认数据源, 这个注解必须要加，因为不加的话spring将分不清楚那个为主数据源（默认数据源）
    @Bean
    @ConfigurationProperties(prefix = "spring.datasource.xytest") //读取application.yml中的配置参数映射成为一个对象
